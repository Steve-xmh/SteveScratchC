...
{
	GObject* o = gtk_builder_get_object(b,n);
	if (!o)
	{
		GtkWidget* msg = gtk_message_dialog_new(NULL,GTK_DIALOG_MODAL,GTK_MESSAGE_ERROR,GTK_BUTTONS_OK,"\n读取窗口布局文件失败：\n无法找到窗口对象：%s\n请不要随意修改布局文件 SSCWindow.glade ！",n);
...
sscCompoments* ssc_widgets_init()
{
	...
	GError* err = NULL;
	if (!gtk_builder_add_from_file(builder,"SSCWindow.glade",&err))
	{
	...
	if(!(compoments->overlayFix  = GTK_WIDGET(gtk_fixed_new()))) return compoments;										// 覆盖框架绝对坐标组件
	...
	gtk_widget_show_all(compoments->window);

	// 窗口叠加层
	gtk_overlay_add_overlay(GTK_OVERLAY(compoments->overlay),GTK_FIXED(compoments->overlayFix));
	gtk_overlay_set_overlay_pass_through(GTK_OVERLAY(compoments->overlay),GTK_FIXED(compoments->overlayFix),TRUE);
	gtk_overlay_reorder_overlay(GTK_OVERLAY(compoments->overlay),GTK_FIXED(compoments->overlayFix),-1);
	gtk_widget_set_halign (GTK_FIXED(compoments->overlayFix), GTK_ALIGN_START);
	gtk_widget_set_valign (GTK_FIXED(compoments->overlayFix), GTK_ALIGN_START);

	gtk_header_bar_set_title(GTK_HEADER_BAR(ssc_get_window_component(builder,"ssc-header-bar")),SSC_FULLNAME);
	...
	gtk_image_set_from_file(GTK_IMAGE(ssc_get_window_component(builder,"ssc-window-icon")),"icons/icon48.png");
    ...
    gtk_notebook_set_current_page(GTK_NOTEBOOK(ssc_get_window_component(builder,"ssc-notebook")),0);
