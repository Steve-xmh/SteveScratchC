...
void ssc_core_init(GError **err)
{
	...
	currentCore->mainThread_ui_q = g_async_queue_new_full(g_free);
	g_mutex_init(&currentCore->mainThread_q_m);
	g_mutex_init(&currentCore->mainThread_ui_q_m);
	...
	while(currentCore->status)
	{
		...
		if(command)
		{
			...
			case SSC_CORE_COMMAND_OPEN_PROJECT:
				ssc_core_load_project((gchar*)g_ptr_array_index(command,1));
				break;
			default:
...
GPtrArray *ssc_core_ui_get_command()
{
	if(!currentCore) return NULL;
	g_mutex_lock(&currentCore->mainThread_ui_q_m);
	guint *dataNum = (guint*)g_async_queue_try_pop(currentCore->mainThread_ui_q);
	GPtrArray *data = NULL;
	if (dataNum)
	{
		data = g_ptr_array_new();
		...
		for (i = 0; i<*dataNum; i++)
		{
			g_ptr_array_add(data,g_async_queue_try_pop(currentCore->mainThread_ui_q));
		}
	}
	g_free(dataNum);
	g_mutex_unlock(&currentCore->mainThread_ui_q_m);
	return data;
}

// 指令格式：指令数量,指令类型,指针(参数)...
void ssc_core_ui_push_command(guint dataNum,...)
{
	if(!currentCore) return;
	va_list arg = NULL;
	va_start(arg,dataNum);
	...
	guint *dNum = g_malloc(sizeof(dNum));
	*dNum = dataNum;
	g_async_queue_push(currentCore->mainThread_ui_q,dNum);

	for(int i = 0; i < dataNum; i++)
	{
		gpointer a = va_arg(arg,gpointer);
		g_async_queue_push(currentCore->mainThread_ui_q,a);
	}
